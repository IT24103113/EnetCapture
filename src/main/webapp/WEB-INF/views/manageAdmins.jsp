...
<section class="gradient-overlay py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white bg-opacity-90 p-8 rounded-lg shadow-md">
            <h2 class="text-2xl font-bold text-gray-800 mb-6">Manage Admins</h2>
            <c:if test="${not empty error}">
                <p class="text-red-500 mb-4">${error}</p>
            </c:if>
            <h3 class="text-lg font-medium text-gray-700 mb-4">Add New Admin</h3>
            <form action="${pageContext.request.contextPath}/admins" method="post" class="mb-8">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="username" class="block text-gray-700 font-medium mb-2">Username</label>
                        <input type="text" id="username" name="username" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
                    </div>
                    <div>
                        <label for="password" class="block text-gray-700 font-medium mb-2">Password</label>
                        <input type="password" id="password" name="password" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
                    </div>
                </div>
                <button type="submit" class="mt-4 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">Add Admin</button>
            </form>
            <h3 class="text-lg font-medium text-gray-700 mb-4">Admin List</h3>
            <c:choose>
                <c:when test="${not empty admins}">
                    <div class="overflow-x-auto">
                        <table class="min-w-full border border-gray-300">
                            ...
                            <tr>
                                <th class="border-b px-4 py-2 text-left text-gray-700">Username</th>
                            ...
                            <c:forEach var="admin" items="${admins}">
                                <tr class="hover:bg-gray-50">
                                    <td class="border-b px-4 py-2">${admin.username}</td>
                                    <td class="border-b px-4 py-2">
                                        <a href="${pageContext.request.contextPath}/admins/edit?username=${admin.username}" class="bg-blue-600 text-white px-3 py-1 rounded-lg hover:bg-blue-700 transition">Edit</a>
                                        <a href="${pageContext.request.contextPath}/admins/delete?username=${admin.username}" class="bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 transition" onclick="return confirm('Are you sure?')">Delete</a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
                ...
                <c:otherwise>
                    <p class="text-gray-700">No admins found.</p>
                </c:otherwise>
            </c:choose>
        </div>
    </div>
