...
<html>
<head>
    <title>Edit Photographer - EnetCapture</title>
    <meta charset="UTF-8">
...
<section class="gradient-overlay py-16">
    <div class="max-w-md mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white bg-opacity-90 p-8 rounded-lg shadow-md">
            <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Edit Photographer</h2>
            <c:if test="${not empty error}">
                <p class="text-red-500 text-center mb-4">${error}</p>
            </c:if>
            <form action="${pageContext.request.contextPath}/photographers/edit" method="post">
                <input type="hidden" name="originalName" value="${photographer.name}">
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 font-medium mb-2">Name</label>
                    <input type="text" id="name" name="name" value="${photographer.name}" readonly class="w-full p-3 border border-gray-300 rounded-lg bg-gray-100">
                </div>
                <div class="mb-4">
                    <label for="rating" class="block text-gray-700 font-medium mb-2">Rating (0-5)</label>
                    <input type="number" id="rating" name="rating" step="0.1" min="0" max="5" value="${photographer.rating}" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
                </div>
                <button type="submit" class="w-full bg-blue-600 text-white p-3 rounded-lg hover:bg-blue-700 transition">Update</button>
            </form>
            <a href="${pageContext.request.contextPath}/photographers" class="block text-center mt-4 text-blue-600 hover:underline">Back</a>
        </div>
    </div>
</section>
